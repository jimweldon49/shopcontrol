...
router.put('/settings',requireAdmin,async(req,res)=>{try{const {data,revision}=req.body;const defaults=model.defaults();if(!data||!Number.isInteger(revision))throw Error('Invalid settings.');const optionalEmptyKeys=['coreRecipients','missedCallEscalation'];for(const key of Object.keys(defaults)){if(!Array.isArray(data[key])||data[key].length>300||(!optionalEmptyKeys.includes(key)&&!data[key].length))throw Error(`Invalid ${key} list.`);if(key==='colors'){if(data[key].some(x=>!x||typeof x.name!=='string'||!x.name.trim()||x.name.length>120||!/^#[0-9a-f]{6}$/i.test(x.color)))throw Error('Invalid card color.');}else if(data[key].some(x=>typeof x!=='string'||!x.trim()||x.length>120))throw Error('Invalid setting value.');}
